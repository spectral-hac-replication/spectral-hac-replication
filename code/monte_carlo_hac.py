...
Aligned with revised manuscript V4 and referee response:

  SECTION 3 (fixed weights):
    * Proposition 3.1: defect-density criterion (continuous)
    * Proposition 3.2: standard lag windows (Bartlett, Parzen, Rectangular)
    * Proposition 3.4: discrete transfer principle (fixed weights)

  SECTION 4 (time-varying bandwidths — central section):
    * Theorem 4.1:     Mixture-convexity preservation
    * Corollary 4.3:   PSD for path-averaged Bartlett (any {m_t})
    * Proposition 4.5:  Non-PSD for hard-cutoff averages
    * Proposition 4.7:  Per-observation vs averaged form discrepancy bound
    * Proposition 4.9:  Pointwise domination by Bartlett within convex class
    * Remark 4.11:      Heuristic: non-PSD probability decay

  Monte Carlo design (responds to referee):
  ...
  Key empirical contrasts:
    1. E3h vs E3s:     same {m_t}, hard cutoff vs taper (Prop 3.2 bridge)
    2. E4h vs E4s:     same {m_t}, plug-in-anchored family (more demanding)
    3. E3s_avg/E4s_avg: path-averaged Bartlett (Cor 4.3 — provably PSD)
    4. E3h_avg/E4h_avg: path-averaged rectangular (Prop 4.5 — NOT PSD)
    5. Bandwidth-path variation analysis (tercile splits on Delta_m)
    6. PSD repair comparison
    7. Optional multivariate stress test (bivariate MS-VAR)
  ...
  Figures generated:
    * Figure_MC1_structural_bridge.png
    * Figure_MC2_failures.png
    * Figure_MC3_inference.png
    * Figure_MC4_variation.png
    * Figure_MC5_repair.png
    * Figure_MC6_multivariate.png
    * Figure_B1_parameter_map.png
    * Figure_B2_certified.png
"""
...
def fmt_prob_ci(p, lo, hi, digits=4):
    if np.isnan(p):
        return "---"
...
# ══════════════════════════════════════════════════════════════════════════════
# 5. CONVEXITY VERIFICATION (Theorem 4.1)
# ══════════════════════════════════════════════════════════════════════════════
...
def verify_theorem41_explicit():
    """
    Verify Theorem 4.1 with the explicit counterexample from Proposition 4.5:
    T=2, m_1=1, m_2=3.
    """
    w_bart_1 = np.array([1.0, 0.5, 0.0, 0.0, 0.0])
    w_bart_3 = np.array([1.0, 0.75, 0.5, 0.25, 0.0])
    w_bart_avg = 0.5 * (w_bart_1 + w_bart_3)
...
def _hac_avg(scores, mt, T, kernel="bartlett"):
    """Generic path-averaged HAC estimator (Algorithm 1 in paper)."""
    ...
    for dgp_name in ["DGP1", "DGP2"]:
        ...
        for T in T_LIST:
            ...
            for r in range(R):
                ...
                # Averaged weights for convexity checks
...
def df_to_latex(df, path, caption, label, col_rename=None, float_fmt="%.4g", escape=False, table_env=True):
    df_out = df.copy()
    if col_rename:
    ...
    for dgp in ["DGP1", "DGP2"]:
        for T in T_LIST:
            for est in est_show:
                ...
                rows.append({
                    "DGP": dgp, "T": T, "Estimator": est,
                    r"$\Pr(\hat f_{\min}<0)$": sv["Pr(fmin<0)"],
                    r"$\Pr(\hat\lambda_{\min}<0)$": sv["Pr(lmin<0)"],
                    r"$\mathbb{E}[\hat f_{\min}]$": sv["E[fmin]"],
                    r"$\mathbb{E}[\hat\lambda_{\min}]$": sv["E[lmin]"],
                    "Mean neg. eig. mass": sv["E[neg_eig_mass]"],
                    "Mean neg. spec. mass": sv["E[spec_neg_mass]"],
                    "Mean disc. to avg.": sv.get("E[disc_to_avg]", np.nan),
                })
    df = pd.DataFrame(rows)
    df.to_csv(OUT / "Table_MC_mechanism.csv", index=False)
    df_to_latex(df, OUT / "Table_MC_mechanism.tex",
                caption=r"Mechanism table: hard cutoff vs taper under shared paths, and per-observation vs path-averaged forms under both the ad hoc and plug-in-anchored path families.",
        ...
        for T in T_LIST:
            for est in est_show:
                ...
                rows.append({
                    ...
                    "NetPower": sv["NetPower"],
                    r"Size5$_{\mathrm{PSD}}$": sv["Size5_PSD"],
                    r"Coverage95$_{\mathrm{PSD}}$": sv["Coverage95_PSD"],
                    r"Size5 $|$ neg. spec.": fmt_prob_ci(sv["Size5|neg_spec"], sv["Size5|neg_spec_lo"], sv["Size5|neg_spec_hi"]),
                    r"Cov95 $|$ neg. spec.": fmt_prob_ci(sv["Cov95|neg_spec"], sv["Cov95|neg_spec_lo"], sv["Cov95|neg_spec_hi"]),
                })
    df = pd.DataFrame(rows)
    df.to_csv(OUT / "Table_MC_inference.csv", index=False)
    df_to_latex(df, OUT / "Table_MC_inference.tex",
                caption=r"Inference table. NetPower = mean(local-power indicator minus null-rejection indicator), so apparent power gains from hard cutoff can be read against size distortion.",
    ...
    for dgp in ["DGP1", "DGP2"]:
        for T in T_LIST:
            for est in est_show:
                ...
                rows.append({
                    "DGP": dgp, "T": T, "Estimator": est,
                    r"$\mathbb{E}[\Delta_m]$": sv.get("E[delta_m]", np.nan),
                    r"$\mathbb{E}[\bar m]$": sv.get("E[mean_m]", np.nan),
                    r"$\mathbb{E}[\mathrm{sd}(m_t)]$": sv.get("E[sd_m]", np.nan),
                    "Selector base": sv.get("selector_base", np.nan),
                    r"Pr(neg. spec. $|$ low $\Delta$)": sv.get("Pr(fmin<0)|low_dm", np.nan),
                    r"Pr(neg. spec. $|$ high $\Delta$)": sv.get("Pr(fmin<0)|high_dm", np.nan),
                    r"Pr(neg. eig. $|$ low $\Delta$)": sv.get("Pr(lmin<0)|low_dm", np.nan),
                    r"Pr(neg. eig. $|$ high $\Delta$)": sv.get("Pr(lmin<0)|high_dm", np.nan),
                })
    df = pd.DataFrame(rows)
    df.to_csv(OUT / "Table_MC_variation.csv", index=False)
    df_to_latex(df, OUT / "Table_MC_variation.tex",
                caption=r"Bandwidth-path variation diagnostics. Failure rates are reported separately for the bottom and top terciles of the within-replication bandwidth range \(\Delta_m\).",
        ...
        for T in T_LIST:
            for est in est_show:
                ...
                rows.append({
                    ...
                    "Coverage95": sv["Coverage95"],
                    r"Coverage95$_{\mathrm{PSD}}$": sv["Coverage95_PSD"],
                    "DeltaCoverage": sv["Coverage95_PSD"] - sv["Coverage95"],
                    "Mean dist. to PSD": sv["E[dist_to_psd]"],
                })
    df = pd.DataFrame(rows)
    ...
    for T in T_LIST:
        for est in est_show:
            ...
            rows.append({
                "T": T, "Estimator": est,
                r"$\Pr(\hat\lambda_{\min}<0)$": sv["Pr(lmin<0)"],
                "Size5 joint": sv["Size5_joint"],
                "Power joint": sv["Power_joint"],
                "NetPower joint": sv["NetPower_joint"],
                r"Size5 joint$_{\mathrm{PSD}}$": sv["Size5_joint_PSD"],
                "Mean dist. to PSD": sv["E[dist_psd]"],
            })
    df = pd.DataFrame(rows)
...
if __name__ == "__main__":
    print("\n" + "=" * 72)
    print("  MONTE CARLO V4: Path-Averaged HAC Estimation")
    print("  Section 3: fixed-weight theory (Prop 3.1–3.4)")
    print("  Section 4: time-varying theory (Thm 4.1, Cor 4.3, Prop 4.5, 4.7, 4.9)")
    print("  Estimators: E0–E4 families (per-obs + path-averaged)")
    print("=" * 72)

    # Verify Theorem 4.1 explicit counterexample (Prop 4.5)
    print("\n[0] Verifying Theorem 4.1 explicit counterexample (Prop 4.5)...")
